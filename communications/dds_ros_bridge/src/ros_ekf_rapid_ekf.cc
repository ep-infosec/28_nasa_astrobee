...
RosEkfToRapid::RosEkfToRapid(const std::string& subscribe_topic,
  ...
  state_supplier_.reset(
      new RosEkfToRapid::StateSupplier(rapid::ext::astrobee::EKF_STATE_TOPIC
        + pub_topic, rapid_pub_name, "AstrobeeEkfStateProfile", ""));

  // start subscriber
  sub_ = nh_.subscribe(subscribe_topic,
                       queue_size,
                       &RosEkfToRapid::MsgCallback, this);
  ...
  ekf_timer_ = nh_.createTimer(ros::Rate(1.0),
                                 &RosEkfToRapid::PubEkf,
                                 this,
                                 false,
                                 false);
}

void RosEkfToRapid::MsgCallback(const ff_msgs::EkfStateConstPtr& msg) {
  ekf_msg_ = msg;
...
void RosEkfToRapid::CopyTransform3D(rapid::Transform3D &transform,
...
void RosEkfToRapid::CopyVec3D(rapid::Vec3d& vec_out,
...
void RosEkfToRapid::PubEkf(const ros::TimerEvent& event) {
  ekf_sent_ = true;
...
void RosEkfToRapid::SetEkfPublishRate(float rate) {
  if (rate == 0) {
    ekf_timer_.stop();
    pub_ekf_ = false;
  } else {
